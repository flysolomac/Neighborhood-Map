...
function initialize() {
     ...
     */
    var atlanta = new google.maps.LatLng(33.7461351, -84.389745);
    /*
     ...
     */
    map = new google.maps.Map(document.getElementById('map-canvas'), mapLocate);
}
...
function yelpCall(searchNear, searchFor) {
     ...
     */
    var auth = {
        consumerKey: "DkEkxhVsYxtxev2cKEcv3Q",
        consumerSecret: "N9NMg4RvC6e2M36HJN69l7a8WHI",
        accessToken: "8cWDzJzc80D7xxhcMpO7GrDlq9i_lIAj",
        accessTokenSecret: "w973vYGixNR_EhxRtUNGcXrfdko",
        serviceProvider: {
    ...
    yAjax(message.action, parameterMap);
}

/*
 *	Ajax OAuth method to get yelp's data
 */
function yAjax(url, ydata) {
    $.ajax({
        'url': url,
        'data': ydata,
        'dataType': 'jsonp',
...
function makeYelpList(d) {
    ...
    if (results.length > 0) {
        ...
        for (var result in results) {
            ...
            /*
             *  Image fail proof if img src is returned undefined
             */
            function good(im) {
                if (im === undefined) {
